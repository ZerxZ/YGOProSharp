...
namespace WindBot.Game.AI.Decks
{
    ...
    public class YubelExecutor : DefaultExecutor
    {
        ...
        public YubelExecutor(GameAI ai, Duel duel) : base(ai, duel)
        {
            ...
            AddExecutor(ExecutorType.Activate, CardId.SPIRIT_GATES, Gate_Revive00Fiend);       // e2: ทิ้งมื�?ชุ�?Fiend 0/0
        ...
        bool _gateWantsRecycle = false;   // กำลังจะกดโหมดเก็�?Continuous
        ...
        // เอา mask ของช่องมอนสเตอร์ที่ลิงก์เราชี�?(เฉพาะ Main Monster Zone 0..4)
        ...
        {
            // e3: มีเลเวล 10 -> เก็�?Continuous Spell จากสุสาน (เน้นเก็�?NIGHTMARE_PAIN)
            ...
            // เลือก Pain ก่อน ถ้าไม่มีค่อยเลือก Paradise ตอ�?select card
            return true;
        }
        private bool Is00FiendId(int id)
        {
            // รายชื่�?0/0 Fiend ที่เราใช้ในเด็คนี้
        ...
        // เลือกเป้าหมายที่จะชุ�?ด้วยลำดับความสำคัญตา�?2.2.x
        private int PlanSpiritGatesReviveTarget()
        {
            ...
            // 2.2.3: ถ้�?DBB โดนขัด/ตันเกมแล้วมี Almiraj �?ชุ�?DBB เพื่อไต่ไ�?Moon
            ...
            // ถ้าไม่มีอะไรชุบได้เลย อาจวางแผ�?discard ให�?Spirit ลงสุสานแล้วค่อยชุบ
            if (!spiritOnBoard && spiritInHand) return CardId.SPIRIT_OF_YUBEL;

            return 0;
        }

        // เลือกใบจะทิ้ง: CSB > Paradise > Terror > (บางกรณ�? Spirit
        private int PickSpiritGatesDiscard(int reviveTargetId)
        {
            // เคสอยากชุ�?Spirit แต่ยังอยู่ในมื�?�?ทิ้ง Spirit เป็�?cost แล้วชุบมันเอง
            ...
            // fallback: เลือกใบที�?"ทิ้งแล้วเจ็บน้อยสุด"
        ...
        {
            // e2: ทิ้งมื�?1 �?SS Fiend 0/0 จากสุสาน
            if (Card.Location != CardLocation.SpellZone) return false;
            ...
            // ต้องมีการ์ดให้ทิ้งอย่างน้อ�?1 ใบ
        ...
        private bool ActTract()
        {
            ...
            // ถ้าเข้�?Fiendsmith line แล้ว -> ไม่ใช้ Tract
        ...
        {
            if (Card.Location != CardLocation.MonsterZone) return false;
            // Quick Link เฉพาะเทิร์นศัตรูช่วง Main เพื่อสร้า�?1 interrupt
            if (Duel.Player != 1) return false;
            ...
            // ส่วนใหญ่เอนจินจะถามเลือกการ์ดฝั่งตรงข้ามที่จะใช้เป็นวัสดุ/หรือไล�?            AI.SelectCard(target);
            return DontSelfNG();
        }
        private bool SSPhantom()
        {
            ...
            // วัตถุดิบใบที�?2 จากสุสานตามที่จัดไว้
        ...
        private bool ActVarudras()
        {
            ...
            // รายการเป้�?(ศัตรูก่อ�?ถ้าไม่มีค่อย fallback)
            List<ClientCard> targetList = GetNormalEnemyTargetList(true, true);
            int desc = ActivateDescription;
            int d1 = Util.GetStringId(CardId.VARUDASN_FINAL_BRINGER, 1); // ใช้ทั้�?e1 (Negate) แล�?e2 (Battle Start destroy)
            ...
            if (desc == d1 && Duel.LastChainPlayer == 1 && Duel.CurrentChain.Count > 0)
            {
                ...
                activatedCardIdList.Add(Card.Id); // แท็กว่าเป็�?e1
                return true; // เอนจินจะจัดกา�?detach เป็�?cost ให้เอง
            }

            // e2: Battle Start ทำลา�?1 / e3: ถูกทำลายแล้วทำลา�?1
        ...
        private bool L4ABOSS()
        {
            ...
            // 3) greedy: เลือกลิงก์ที่มีอยู่ก่อน (prefer Rage > Yama > อื่น �? แล้วเติมการ์ดจนแต้มรวมครบ 4
        ...
        private ClientCard[] GetSafeMaterialsExcluding(HashSet<int> excludeIds, int need)
        {
            return Bot.GetMonsters()
                .Where(m => m != null
                            ...
                            && !IsProtectedMaterial(m)      // ไม่ยอมแลกของสำคั�?(Link�? / Extra ฯล�?
                       )
                .OrderBy(m => PriorityIndex(m.Id))
                .ThenBy(m => m.Attack)
                .Take(need)
        ...
        {
            // ต้องมี Yama อยู่ก่อน และต้องมีวัตถุดิบอื่�?2 ใบ (ไม่นับ Yama)
        ...
        public override bool OnSelectYesNo(int desc)
        {
            ...
            { return false; }
            // --- Nightmare Throne ---
            // ͨ�� desc �� solving ��ͬê�� Nightmare Throne ����ʾ��Դ��
            if (solving != null && solving.IsCode(CardId.NIGHTMARE_THRONE))
            {
                // ��� desc map������ȷ�� index ��Ӧ��ϵ��
                DebugThroneDescMap(desc);

                // ยังไม่ได้เริ่มค้�?-> ตอ�?YES เพื่อเข้าโหมดค้น
                ...
                // ค้นเสร็จแล้ว และกำลังถา�?"จะทำลายไหม?" -> default = NO
                ...
                // กันเคส engine ถามซ้ำก่อนโชว์เด็�? ตอ�?YES ไป (จะไปเข้�?OnSelectCard)
            ...
            // --- Varudras: ถามถอดวัตถุดิบอี�?1 เพื่อทำลา�?---
        ...
        public override IList<ClientCard> OnSelectCard(IList<ClientCard> cards, int min, int max, int hint, bool cancelable)
        {
            ...
            if (cards != null && cards.Count > 0)
            {
                ...
                if (_throneStage == ThroneStage.Searching && solving != null && solving.IsCode(CardId.NIGHTMARE_THRONE) && !throneSearched && cards != null && cards.Count > 0)
                {
                    ...
                    // fallback������ѡ Yubel ϵ�к�ѡ������ʹ�õ�һ����ѡ��
                    if (chosen == null)
                ...
                if (_yubelWantsTribute && isReleasePrompt && cards != null && cards.Count > 0)
                {
                    ...
                    // เผื่อกรณีเอนจินส่งลิสต์มาแต่ดันไม่มี Lotus (ไม่ควรเกิ�?เพราะเราตอ�?YES เมื่อมี Lotus)
                ...
                {
                    // พยายามเลือกฝั่งศัตรูก่อ�?(คัดใบที่อันตรา�?ป่วนที่สุด)
                    var enemyPick = cards
                ...
                if (solving != null && solving.IsCode(CardId.UNCHAINED_SOUL_OF_RAGE) && cards.Any(c => c != null && c.Location == CardLocation.Extra))
                {
                    ...
                    if (pickGorgon != null)
                    {
                        ...
                        // EMZ ������ʱ��Ҫ����ѡ Gorgon�������� fallback��
                    }

                    // 3) fallback��ʹ�õ�һ����ѡ���ϡ�
