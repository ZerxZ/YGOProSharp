...
namespace WindBot.Game
{
    public class GameBehavior
    {
        ...
        public INamedCardRepository CardRepository { get; }
        ...
        public GameBehavior(GameClient game, INamedCardRepository cardRepository)
        {
            Game = game;
            Connection = game.Connection;
            CardRepository = cardRepository ?? EmptyNamedCardRepository.Instance;
            _hand = game.Hand;
            _debug = game.Debug;           
            ...
            Deck = Deck.Load(Game.DeckFile ?? _ai.Executor.Deck, CardRepository);
        ...
        private void OnErrorMsg(BinaryReader packet)
        {
            ...
            if (msg == 2) //ERRMSG_DECKERROR
            {
                ...
                {
                    NamedCard card = CardDatabase.Get(code);
                    if (card != null)
        ...
        private void OnMove(BinaryReader packet)
        {
            ...
            if ((previousLocation & (int)CardLocation.Overlay) != 0)
            {
                ...
                {
                    if (_debug)
                        Logger.WriteLine("(" + previousControler.ToString() + " 's " + (card.Name ?? "UnKnowCard") + " deattach " + (CardDatabase.Get(cardId)?.Name) + ")");
            ...
            if ((currentLocation & (int)CardLocation.Overlay) != 0)
            {
                ...
                {
                    if (_debug)
                        Logger.WriteLine("(" + previousControler.ToString() + " 's " + (card.Name ?? "UnKnowCard") + " overlay " + (CardDatabase.Get(cardId)?.Name) + ")");
            ...
            else
            {
                ...
                {
                    if (_debug)
                        Logger.WriteLine("(" + previousControler.ToString() + " 's " + (CardDatabase.Get(cardId)?.Name)
        ...
        private void OnAnnounceCard(BinaryReader packet)
        {
            ...
            IList<NamedCard> all = CardDatabase.GetAllCards().ToList();
